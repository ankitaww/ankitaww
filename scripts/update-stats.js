...
async function main() {
  ...
  const query = `query { user(login: "${username}") { followers { totalCount } following { totalCount } repositories(privacy: PUBLIC) { totalCount } contributionsCollection { totalCommitContributions } } }`;
  ...
  const u = body.data.user, commits = u.contributionsCollection.totalCommitContributions;
  const total = commits;
  ...
  let svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 520 110" width="520" height="110"><rect width="520" height="110" rx="12" fill="#1a1b26"/><text x="24" y="30" fill="#bb9af7" font-family="sans-serif" font-size="16" font-weight="bold">${username} · GitHub activity</text>`;
  ...
  fs.writeFileSync('stats.svg', svg + '</svg>'); console.log(`Updated stats for ${username}`);
}
